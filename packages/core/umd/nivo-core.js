(function (global, factory) {
  ...
  var DotsItem = function (_React$Component) {
      inherits(DotsItem, _React$Component);

      function DotsItem(props) {
          classCallCheck(this, DotsItem);

          var _this = possibleConstructorReturn(this, _React$Component.call(this, props));

          _this._dotSymbol = React__default.createRef();
      ...
      DotsItem.prototype.componentDidMount = function componentDidMount() {
          if (this.props.setHover && this._dotSymbol.current) {
              this.setUpListeners();
          }
      };

      DotsItem.prototype.componentWillUnmount = function componentWillUnmount() {
          if (this.props.setHover && this._dotSymbol.current) {
              this._dotSymbol.current.removeEventListener("mouseover", this.showHoverPanel);
              this._dotSymbol.current.removeEventListener("mouseout", this.clearHoverTracking);
          }
      };

      DotsItem.prototype.setUpListeners = function setUpListeners() {
          this._dotSymbol.current.addEventListener("mouseover", this.showHoverPanel);
          this._dotSymbol.current.addEventListener("mouseout", this.clearHoverTracking);
      };

      DotsItem.prototype.showHoverPanel = function showHoverPanel(e) {
          this._dotSymbol.current.addEventListener("mousemove", this.moveHoverPanel);
          this.moveHoverPanel(e);
      };

      DotsItem.prototype.clearHoverTracking = function clearHoverTracking() {
          this.props.setHover && this.props.setHover(null, this.props.datum, false);
          this._dotSymbol.current.removeEventListener("mousemove", this.moveHoverPanel);
      };

      DotsItem.prototype.moveHoverPanel = function moveHoverPanel(e) {
          var _props = this.props,
              datum = _props.datum,
              color = _props.color;

          this.props.setHover(e, _extends({}, datum, { color: color }), true);
      };

      DotsItem.prototype.render = function render() {
          var _props2 = this.props,
              ...
              label = _props2.label,
              labelTextAnchor = _props2.labelTextAnchor,
              labelYOffset = _props2.labelYOffset,
              theme = _props2.theme,
              setHover = _props2.setHover;

          return React__default.createElement(
              'g',
              { transform: 'translate(' + x + ', ' + y + ')', style: { pointerEvents: setHover ? 'auto' : 'none' }, ref: this._dotSymbol },
